...
用硬件红绿灯显示 **Claude Code** / **OpenAI Codex** / **Cursor** 的工作状态（ESP32-C3 + 三色灯）。  
...
- 桌面调试台：`pythonw tools/vibelight_gui.py` / 双击 `tools/vibelight_gui.pyw`（无控制台推荐）或 `python tools/vibelight_gui.py`（[tools/README.md](tools/README.md)）；Hook×灯效配置：`pythonw tools/tl_hook_light_gui.py` / `tools/tl_hook_light_gui.pyw` 或 `python tools/tl_hook_light_gui.py`（见 [docs/HOOK_EVENTS_REFERENCE.md](docs/HOOK_EVENTS_REFERENCE.md)）
