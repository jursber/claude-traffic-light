...
def start_via_task():
    ...
    计划任务 "ClaudeTrafficLightDaemon" 由 extras/legacy_windows/install_service.py 创建，
    配置为登录时自动启动、隐藏窗口。
    这里手动触发一次运行。
    """
...
def main():
    ...
    if is_daemon_running():
        ...
        print("启动失败。请以管理员身份运行 extras/legacy_windows/install_service.py 注册计划任务。", file=sys.stderr)
        sys.exit(1)
